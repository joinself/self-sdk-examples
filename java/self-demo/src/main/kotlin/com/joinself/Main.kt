...
class DemoApp {
    ...
    var target = Target.productionSandbox()
    ...
    suspend fun run() {
        ...
        val callbacks = Callbacks(
            ...
            onKeyPackage = { account, keyPackage: KeyPackage ->
                ...
                account.connectionEstablish(
                    asAddress = keyPackage.toAddress(), keyPackage = keyPackage.keyPackage(),
            ...
            onWelcome = { account, welcome: Welcome ->
                println("KMP welcome")
                account.connectionAccept(
                    asAddress = welcome.toAddress(),
                    welcome = welcome.welcome()
                ) { status: SelfStatus, gAddress: PublicKey ->
                    println(
                        "accepted connection encrypted group status:${status.name()} - from:${
                            welcome.fromAddress().encodeHex()
                        } - group:${gAddress.encodeHex()}"
                    )
            ...
            onMessage = { account, message: Message ->
                ...
                when (contentType) {
                    ContentType.DISCOVERY_RESPONSE -> {
                        ...
                        println(
                            "received response to discovery request from:${
                                message.fromAddress().encodeHex()
                            } - requestId:${responseTo} - messageId:${message.id().toHexString()}"
                        )
            ...
            onIntegrity = { account, integrity: Integrity ->
                ...
                Attestation.deviceCheck(
                    applicationAddress = PublicKey.decodeHex("0016fced9deea88223b7faaee3e28f0363c99974c67ee7842ead128a0f36a9f1e3"),
                    integrityToken = ByteArray(integrity.requestHash().size + 128)
                )
            }
        )
...
//        .documentAddress(documentAddress)
        ...
        if (target.environment().contains("sandbox")) {
    ...
    private fun sendAgreementRequest(account: Account) {
        ...
        println(
            "send agreement status:${sendStatus.name()} - to:${responderAddress!!.encodeHex()} - requestId:${
                agreementRequest.id().toHexString()
            }"
        )
    }
